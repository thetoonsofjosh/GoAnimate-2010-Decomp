package anifire.core
{
   ...
   public class AnimeSound
   {
      ...
      private var _soundThumb:anifire.core.SoundThumb;
      ...
      private function doPlayAgain(param1:Event) : void
      {
         (param1.target as IEventDispatcher).removeEventListener(param1.type,this.doPlayAgain);
         this.playSound();
      }
      ...
      public function get soundThumb() : anifire.core.SoundThumb
      {
         return this._soundThumb;
      }
      
      private function setSoundThumb(param1:anifire.core.SoundThumb) : void
      {
      ...
      {
         return Console.getConsole().timeline.getSoundIndexById(this.getID());
      }
      ...
      public function init(param1:anifire.core.SoundThumb, param2:Number, param3:Number, param4:String = null, param5:Number = 0, param6:Number = 1) : void
      {
         var _loc8_:int = 0;
         this.setSoundThumb(param1);
         var _loc7_:String = param4 != null ? param4 : "SOUND" + AnimeSound._existIDs.length;
         if(param4 == null)
         {
            _loc8_ = AnimeSound._existIDs.length + 1;
            while(AnimeSound._existIDs.containsKey(_loc7_))
            {
               _loc7_ = "SOUND" + _loc8_;
               _loc8_++;
            }
         }
         AnimeSound._existIDs.push(_loc7_,_loc7_);
         this.setID(_loc7_);
      ...
      public function playSound(param1:Number = 0) : void
      {
         ...
         {
            this.stopSound();
            this.sound.play(param1,this.soundContainer);
            Console.getConsole().thumbTray.stopAllSounds();
         }
         else
         {
            this.sound = this.getSoundThumb().requestSoundObjCopy();
            this.sound.addEventListener(SoundEvent.READY_TO_PLAY,this.doPlayAgain);
            anifire.core.SoundThumb.initSoundObj(this.sound,this.getSoundThumb());
         }
      }
      ...
      public function serialize(param1:Boolean) : String
      {
         var _loc2_:* = "";
         if(this.endFrame > this.startFrame)
         {
            _loc2_ += "<sound id=\"" + this.getID() + "\" index=\"" + this.index + "\" track=\"" + this.trackNum + "\" vol=\"" + this.inner_volume + "\">";
            _loc2_ += "<sfile>" + this.getSoundThumb().theme.id + "." + this.getSoundThumb().id + "</sfile>";
            _loc2_ += "<start>" + this.startFrame + "</start>";
            _loc2_ += "<stop>" + this.endFrame + "</stop>";
            _loc2_ += "</sound>";
            if(param1)
            {
      ...
      public function showMenu(param1:Number, param2:Number, param3:SoundContainer = null) : void
      {
         ...
         _loc4_ += "</menuItem>" + "<menuItem label=\"\" type=\"separator\"/>" + "<menuItem label=\"" + UtilDict.toDisplay("go",MENU_LABEL_REMOVE_SOUND) + "\" value=\"" + MENU_LABEL_REMOVE_SOUND + "\" /></root>";
         _loc5_ = new XML(_loc4_);
         (_loc6_ = ScrollableArrowMenu.createMenu(null,_loc5_,false)).setParentObject(this.getSceneCanvas());
         _loc6_.labelField = "@label";
      ...
      private function getSoundThumb() : anifire.core.SoundThumb
      {
         return this._soundThumb;
      }
