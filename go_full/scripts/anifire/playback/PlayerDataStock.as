package anifire.playback
{
   ...
   public class PlayerDataStock extends EventDispatcher
   {
      ...
      private function onLoadMovieZipCompleted(param1:Event) : void
      {
         ...
         var ccFileHashArray:UtilHashArray = null;
         var args:Object = null;
         var ccXML:XML = null;
         var should_decrypt:Boolean = false;
            ...
            while(i < zipFile.size)
            {
               ...
               else if(zipEntry.name.substr(zipEntry.name.length - 3,3).toLowerCase() == "zip")
               {
                  ...
                  while(j < ccZipFile.size)
                  {
                     ...
                     if(ccZipEntry.name == PlayerConstant.CHAR_XML_FILENAME)
                     {
                        ccXML = new XML(ccZipFile.getInput(ccZipEntry).toString());
                     }
                     else if(ccZipEntry.name.substr(ccZipEntry.name.length - 3,3).toLowerCase() == "swf")
                     {
                        ccFileBytes = ccZipFile.getInput(ccZipEntry);
                        trace("ccZipEntry.name:" + [ccZipEntry.name,ccFileBytes.bytesAvailable]);
                        should_decrypt = true;
                        if(should_decrypt)
                        {
                  ...
                  this.addPlayerData(zipEntry.name,args);
               }
               else
               {
                  fileBytes = zipFile.getInput(zipEntry);
