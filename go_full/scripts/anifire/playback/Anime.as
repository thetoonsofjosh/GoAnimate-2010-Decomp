package anifire.playback
{
   ...
   public class Anime
   {
      ...
      private var _dataStock:anifire.playback.PlayerDataStock = null;
      ...
      private var _assetSound:anifire.playback.AssetSound;
      ...
      private var _downloadManager:anifire.playback.DownloadManager;
      ...
      public function Anime()
      {
         ...
         this._downloadManager = new anifire.playback.DownloadManager();
         this._startFrameOffsetForEachMovie = new UtilHashArray();
         this._movieInfo = {};
         super();
         this.setDataStock(new anifire.playback.PlayerDataStock());
      ...
      private function goToAndPause(param1:Number, param2:Boolean = false) : void
      {
         ...
         trace("----> Anime: gotoAndpause: " + param1);
      ...
      private function get downloadManager() : anifire.playback.DownloadManager
      {
         return this._downloadManager;
      }
      
      protected function init(param1:anifire.playback.PlayerDataStock, param2:Boolean = false) : void
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         ...
         this._assetSound = new anifire.playback.AssetSound(this);
         this.setDataStock(param1);
         this.downloadManager = new anifire.playback.DownloadManager();
         this.downloadManager.init();
         var _loc6_:int = 0;
         while(_loc6_ < param1.getFilmXmlArray().length)
         {
            _loc7_ = param1.getFilmXmlArray()[_loc6_] as XML;
            _loc8_ = XML(_loc7_.child("meta")[0]).child("mver")[0] != null ? Number(XML(_loc7_.child("meta")[0]).child("mver")[0]) : 0;
            ...
            {
               if((_loc11_ = _loc9_[_loc3_] as XML).toXMLString().toLowerCase().indexOf(AnimeConstants.MAGIC_KEY) <= -1)
               {
      ...
      private function set downloadManager(param1:anifire.playback.DownloadManager) : void
      {
      ...
      protected function setDataStock(param1:anifire.playback.PlayerDataStock) : void
      {
      ...
      private function initDependency() : void
      {
         ...
         if(_loc2_ > 0)
         {
            ...
            while(_loc6_ < _loc2_)
            {
               _loc7_ = _loc6_ < _loc2_ - 1 ? this.getSceneByIndex(_loc6_ + 1) : null;
               _loc8_ = _loc6_ > 0 ? this.getSceneByIndex(_loc6_ - 1) : null;
               this.getSceneByIndex(_loc6_).initDependency(_loc7_,_loc8_,_loc1_ + 1,this.downloadManager,_loc3_);
               _loc1_ += this.getSceneByIndex(_loc6_).getDuration();
               _loc6_++;
            }
         ...
         var _loc4_:int = 0;
         while(_loc4_ < this.getNumSounds())
         {
            (_loc10_ = this.getSoundByIndex(_loc4_)).initDependency(this.startFrameOffsetForEachMovie.getValueByKey(_loc10_.movieId) as Number,this.movieDurationPerMovie.getValueByKey(_loc10_.movieId) as Number,this.downloadManager);
            if(!this._soundsIndexByStartFrame.containsKey(_loc10_.getStartFrame().toString()))
            {
               this._soundsIndexByStartFrame.push(_loc10_.getStartFrame().toString(),new Array());
            }
            (_loc9_ = this._soundsIndexByStartFrame.getValueByKey(_loc10_.getStartFrame().toString()) as Array).push(_loc10_);
            if(!this._soundsIndexByEndFrame.containsKey(_loc10_.getEndFrame().toString()))
            {
               this._soundsIndexByEndFrame.push(_loc10_.getEndFrame().toString(),new Array());
            }
            (_loc9_ = this._soundsIndexByEndFrame.getValueByKey(_loc10_.getEndFrame().toString()) as Array).push(_loc10_);
            _loc11_ = _loc10_.getEndFrame();
            _loc12_ = _loc10_.getStartFrame();
            while(_loc12_ <= _loc11_)
            {
               if(!this._soundsIndexByFrame.containsKey(_loc12_.toString()))
               {
                  this._soundsIndexByFrame.push(_loc12_.toString(),new Array());
               }
               (_loc9_ = this._soundsIndexByFrame.getValueByKey(_loc12_.toString()) as Array).push(this.getSoundByIndex(_loc4_));
               _loc12_++;
            }
            _loc4_++;
         }
         this.downloadManager.initDependency(UtilUnitConvert.frameToSec(this.getDuration() * 1000),this.getMovieContainer().stage,this);
         this.downloadManager.startDownload();
         if(this.downloadManager.getIsBufferReadyByTimeRange(0,AnimeConstants.MIN_TIME_TO_BUFFER))
         {
            this.doDispatchMovieStartBufferReady(null);
         }
         else
         {
            ...
            trace("#########");
            trace("add BUFFER_READY listener: doDispatchMovieStartBufferReady");
         }
         var _loc5_:PlayerEvent = new PlayerEvent(PlayerEvent.MOVIE_STRUCTURE_READY);
      ...
      private function traceMemory(param1:Number, param2:String) : void
      {
         ...
         {
            trace("enterframe, " + param2 + ":" + param1.toFixed(2));
         }
         this.lastNum = param1;
      }
      ...
      private function onEnterFrame(... rest) : void
      {
         ...
         else
         {
            ...
            {
               _loc20_ = UtilUnitConvert.frameToSec(this._currentFrame) * 1000;
               _loc5_ = 0;
               while(_loc5_ < _loc4_.length)
               {
                  (_loc16_ = _loc4_[_loc5_] as AnimeSound).play(_loc20_);
                  _loc5_++;
               }
            }
      ...
      private function initFonts() : void
      {
         ...
         if(this.getNumFonts() > 0)
         {
            ...
            {
               _loc2_ = String(this._fonts[_loc3_]);
      ...
      protected function getDataStock() : anifire.playback.PlayerDataStock
      {
