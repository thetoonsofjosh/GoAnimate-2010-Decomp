package anifire.timeline
{
   ...
   public class TimelineHScrollBar extends HScrollBar
   {
      ...
      private var _maxWidthContainer:anifire.timeline.ITimelineContainer;
      
      private var _conatainerArr:ArrayCollection;
      
      private var _changeType:String;
      
      private var _currContainer:anifire.timeline.ITimelineContainer;
      
      private var background_bg:anifire.timeline.BackgroundGrid;
      ...
      public function addContainerToScrollBar(param1:anifire.timeline.ITimelineContainer) : void
      {
      ...
      private function setContainerView(param1:Event) : void
      {
         ...
         var _loc4_:Number = this.scrollPosition == 0 ? 0 : (_loc2_ - _loc3_) * this.scrollPosition / this.maxScrollPosition;
      ...
      public function initBgGrid(param1:anifire.timeline.BackgroundGrid) : void
      {
         this.background_bg = param1;
      }
      
      private function getChildTotalWidth(param1:anifire.timeline.ITimelineContainer) : Number
      {
      ...
      {
         var _loc3_:int = int(this._conatainerArr.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
      ...
      override public function set visible(param1:Boolean) : void
      {
         ...
         var _loc6_:anifire.timeline.ITimelineContainer = null;
         ...
         if(param1 == true)
         {
            if(this._currContainer is SceneContainer && this._changeType != "Resize")
            {
               ...
               if((_loc4_ = (this._currContainer as SceneContainer).getCurrIndex()) != -1)
               {
                  ...
                  {
                     if((_loc5_ = this.maxScrollPosition * (_loc2_ + this._currItem.width + 30 - this.width) / (this._conatainerArr.getItemAt(this._conatainerArr.length - 1).Width - this.width)) < 0)
                     {
                        _loc5_ = 0;
                     }
            ...
            else if(this._currContainer is SceneContainer && this._changeType == "Resize")
            {
               ...
               this.lineScrollSize = this._conatainerArr.getItemAt(this._conatainerArr.length - 1).Width / 100;
               this.dispatchEvent(new Event("SCROLL_VISIBLE"));
               _loc7_ = (_loc6_ = this._conatainerArr.getItemAt(this._conatainerArr.length - 1).Container as anifire.timeline.ITimelineContainer).getHorizontalView();
               if(!isNaN(_loc7_))
               {
                  _loc8_ = 0 - _loc7_;
      ...
      {
         this._currContainer = param1.getEventCreater() as anifire.timeline.ITimelineContainer;
         this._changeType = param1.getData() as String;
         this._currItem = this._currContainer.getCurrItem();
         var _loc2_:int = 0;
         while(_loc2_ < this._conatainerArr.length)
         {
            if(this._conatainerArr.getItemAt(_loc2_).Container.name == (param1.currentTarget as anifire.timeline.ITimelineContainer).name)
            {
               this._conatainerArr.setItemAt({
                  "Container":param1.currentTarget,
                  "Width":this.getChildTotalWidth(param1.currentTarget as anifire.timeline.ITimelineContainer)
               },_loc2_);
            }
            _loc2_++;
         }
      ...
      private function set maxWidthContainer(param1:anifire.timeline.ITimelineContainer) : void
      {
